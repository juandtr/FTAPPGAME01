...
const GameTable = ({ onExitTable }) => {
  ...
  const handlePress = (item, index, array) => {
    ...
    array[index].emoji = currentPlayer === 'Juan' ? '' : (change > 0 ? '😀' : '😡');
    ...
    setTimeout(() => {
      ...
      if (availableItems.length > 1) {
        ...
        randomIndexes.forEach(randomIndex => {
          ...
          handlePress(item, index, item === '💎' ? diamondStates : item === '💰' ? goldBarStates : item === '🔴' ? rubyStates : trophyStates);
  ...
  return (
    <ImageBackground source={require('../assets/FTAPPGAME (5).png')} style={styles.background}>
        ...
        <View style={styles.gameTable}>
          ...
          <View style={styles.row}>
            {diamondStates.map((diamond, index) => (
              <TouchableOpacity
                key={index}
                style={[getCellStyle(diamond.available, currentPlayer === 'Juan'), { backgroundColor: diamond.emoji === '☀️' ? 'green' : (diamond.available ? '#00F' : '#888') }]}
            ...
            {goldBarStates.map((goldBar, index) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  { backgroundColor: goldBar.emoji === '🥇' ? 'red' : (goldBar.available ? '#FFD700' : '#888') }
                ]}
          ...
          <View style={styles.row}>
            {rubyStates.map((ruby, index) => (
              <TouchableOpacity
                key={index}
                style={[getCellStyle(ruby.available, currentPlayer === 'Juan'), { backgroundColor: ruby.emoji === '🍀' ? 'yellow' : (ruby.available ? 'red' : '#888') }]}
          ...
          <View style={styles.row}>
            {trophyStates.map((trophy, index) => (
              <TouchableOpacity
                key={index}
                style={[getCellStyle(trophy.available, currentPlayer === 'Juan'), { backgroundColor: trophy.emoji === '🏆' ? 'blue' : (trophy.available ? 'green' : '#888') }]}
      ...
      <View style={styles.infoContainer}>
        ...
        {juanIsPlaying && <Text style={styles.infoText}><Text style={{ fontWeight: 'bold', color: 'blue',  fontSize: 20  }}>Juan está jugando...</Text></Text>}
